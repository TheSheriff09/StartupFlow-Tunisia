...
public class StartupService implements ICRUD<Startup> {
    ...
    public Startup add(Startup s) {
        String sql = "INSERT INTO startup " +
                ...
                " mentorID, fundingAmount, incubatorProgram, founderID, businessPlanID) " +
                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ...
        try (PreparedStatement ps = cnx.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (s.getKpiScore() != null) ps.setDouble(6, s.getKpiScore()); else ps.setNull(6, Types.DOUBLE);
            ...
            if (s.getMentorID() != null) ps.setInt(10, s.getMentorID()); else ps.setNull(10, Types.INTEGER);
            if (s.getFundingAmount() != null) ps.setDouble(11, s.getFundingAmount()); else ps.setNull(11, Types.DOUBLE);
            ps.setString(12, s.getIncubatorProgram());
            if (s.getFounderID() != null) ps.setInt(13, s.getFounderID()); else ps.setNull(13, Types.INTEGER);
            if (s.getBusinessPlanID() != null) ps.setInt(14, s.getBusinessPlanID()); else ps.setNull(14, Types.INTEGER);

            ps.executeUpdate();
            try (ResultSet keys = ps.getGeneratedKeys()) {
                if (keys.next()) s.setStartupID(keys.getInt(1));
            }
